...
namespace QLCoffee
{
    public partial class FormQLSanPham : Form
    {
        ...
        private void txttimkiem_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                var lschange = db.Coffes.Where(t => (t.TenSanPHam.ToLower().Trim().Contains(txttimkiem.Text.ToLower()) || t.Gia.ToString().Contains(txttimkiem.Text))).ToList();
                coffeDTOBindingSource.DataSource = lschange;
            }
            else
            {
                loadData();
